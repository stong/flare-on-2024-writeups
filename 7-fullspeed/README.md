...
```
- the curve parameters are shitty, it has a large nonprime subgroup but cofactor is not handled properly. shoutout to Chatgpt on this one, i was like "idk just write me a sage script to audit these parameters for problems" and i didnt even have to use my brain. (cofactor is basically "ok this are the shitty small primes subgroup in our group, don't use those, use only the large prime subgroup for our crypto").
```c
...
```
- curve order is not prime and in fact it's in [factordb](https://factordb.com/index.php?query=30937339651019945892244794266256713890440922455872051984762505561763526780311616863989511376879697740787911484829297)
...
- BUT THERE IS STILL A LARGE-ISH PRIME SUBGROUP in the factorization :((( ![image](https://github.com/user-attachments/assets/fec0e901-a671-44a7-8cbb-594850bf701a)
- oh well, do it modulo all the subgroup made up of other small primes subgroups that ARE small enough for us to factor. so lets say we have small primes $p,q,r,\cdots$ and we multiply $p\times q\times r \times \cdots = N$. Then we can at least know that the dlog $\equiv$ x (mod N) (credit to chatgpt for this idea and all the sage chode)

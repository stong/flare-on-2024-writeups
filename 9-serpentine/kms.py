...
def doit():
    ...
    def emu_halt(uc, address, size, user_data):
            ...
            while index < unwind_info.CntUnwindCodes:
                ...
                if prolog_offset >= unwind_code.PrologOff:
                    if unwind_op == UWOP_PUSH_NONVOL:
                        ...
                        block_disasm.append((0, 0, 0, 'mov', f'Context{reg_name}, qword ptr [rdx]'))
                        ...
                        block_disasm.append((0, 0, 0, 'mov', f'rdx, Context{new_frame_regnam.lower()}'))
                        if frame_offset != 0:
        ...
        for regname, regunicorn in zip(reg_nums_to_names,reg_nums_to_unicorn):
            ...
            block_disasm.append((0, 0, 0, 'mov', f'context{regname.lower()}, {regname.lower()}'))
        
        mem_write_u32(pCONTEXT+0x34, mu.reg_read(UC_X86_REG_MXCSR))
        block_disasm.append((0, 0, 0, 'mov', 'contextmxcsr, mxcsr'))
    ...
    def make_operands_less_stupid(disasm):
        assert len(disasm) == 1
        tup = disasm[0]
        mnemonic = tup[2]
        operands = tup[3].split(', ')
        for i in range(len(operands)):
            if operands[i] == 'qword ptr [r9 + 0x28]': # this is loading the CONTEXT pointer that points to our previous registers that got spilled in during context switch from exception handling
                # we should not include this in the devirtualized output
                # this will clobber the real register, this code is running in the context of the exception handler
                # and we are de-exception-handler-ing the code
                operands = []
                mnemonic = 'nop'
                break
            if m := re.match(r'^((d|q)word) ptr \[(\w+) \+ (0x[0-9a-f]{1,2})\]$', operands[i]):
                 ptrsize, basereg, offset = m[1], m[3], m[4]
                 offset = int(offset,16)
                 if mu.reg_read(names_to_unicorn[basereg]) == pCONTEXT:
                    new_regname = context_layout[offset].lower()
                    if ptrsize == 'dword':
                        new_regname = regnames_to_32bits_names[new_regname]
                    operands[i] = 'context' + new_regname
        if mnemonic == 'ldmxcsr':
            assert len(operands) == 1
            mnemonic = 'mov'
            # operands = [f'dword ptr [0x{pMxcsr:08X}]', operands[0]] # just spill this shit to memory
            operands = ['mxcsr', operands[0]] # uhhh let the reg allocator take care of it ig
        tup = (*tup[0:2], mnemonic, ', '.join(operands))
        return tup
    ...
    def hook_code64(uc, address, size, user_data):
        ...
        if address == pLoseFunc:
            ...
            return False
        if address == 0x1400011B0:
            print("we are at the win func")
            mu.reg_write(UC_X86_REG_RIP, 0xDEADBEEFDEADBEEF)
            return False
        ...
        if len(disasm) == 1:
            ...
            if mnemonic == 'hlt':
                ...
                return False
            if mnemonic == 'cmovne' or mnemonic == 'test':
                print("%x"%mu.reg_read(UC_X86_REG_RFLAGS))
                mu.reg_write(UC_X86_REG_RFLAGS, mu.reg_read(UC_X86_REG_RFLAGS)|0x40)
                print("fuck\n\n\n\n\n")
            if is_new_code and mnemonic == 'jmp':
                print("skipping dis shit!")
                mu.reg_write(UC_X86_REG_RIP, mu.reg_read(UC_X86_REG_RIP)+disasm_size)
    ...
    is_new_code = len(sys.argv) > 1 and sys.argv[1] == 'new_code.bin'
    if not is_new_code:
    ...
    if not 'no_stack' in ' '.join(sys.argv):
        # map a stack
        stack_base, stack_size = 0x7ffffff0000, 0x800000
        mu.mem_map(stack_base, stack_size)
    else:
        print('WILL NOT ALLOCATE A STACK!')
        stack_base, stack_size = 0,0
        mu.reg_write(UC_X86_REG_RSP, 0xBADBADBADBADBAD0)
    ...
    mu.mem_write(pFlagBuf, b'00000000000000000000000000000000')
    ...
    def clean_disasm(block_disasm):
        ...
        while i < len(block_disasm):
            for fucker_pat in fucker_pats:
                if (matched_instrs := fucker_match(fucker_pat, block_disasm[i:i+len(fucker_pat[0])])) is not None:
    ...
    def get_def_operands(mnemonic, operands):
        if mnemonic in ['mov','movabs','add','movzx','shl','shr','and','not','or','sub','xor','lea','cmovne']:
            return [operands[0]]
    ...
    def get_use_operands(mnemonic, operands):
        if mnemonic in ['mov','movabs','movzx','lea']:
            ...
            return [operands[1]]
        elif mnemonic in ['add','shl','shr','and','or','sub','xor']:
            assert len(operands) == 2
    ...
    def global_value_numbering(block_disasm):
        ...
        the_32bit_regs = {
            'contextrax': 'contexteax',
            'contextrbx': 'contextebx',
            'contextrcx': 'contextecx',
            'contextrdx': 'contextedx',
            'contextrsi': 'contextesi',
            'contextrdi': 'contextedi',
            'contextrbp': 'contextebp',
            'contextrsp': 'contextesp',
            'contextr8': 'contextr8d',
            'contextr9': 'contextr9d',
            'contextr10': 'contextr10d',
            'contextr11': 'contextr11d',
            'contextr12': 'contextr12d',
            'contextr13': 'contextr13d',
            'contextr14': 'contextr14d',
            'contextr15': 'contextr15d',
            'mxcsr': 'mxcsr',
            'contextmxcsr': 'contextmxcsr',
        }
        ...
        result = []
        number = 0
        register_versions = {k:0 for k in ['contextrax', 'contextrbx', 'contextrcx', 'contextrdx', 'contextrsi', 'contextrdi', 'contextrbp', 'contextrsp', 'contextr8', 'contextr9', 'contextr10', 'contextr11', 'contextr12', 'contextr13', 'contextr14', 'contextr15', 'contextmxcsr', 'mxcsr']}

        needs_value_numbering = lambda regname: regname in register_versions
        for addr, field1, field2, mnemonic, op_str in block_disasm:
            ...
            for def_reg in def_regs:
                if needs_value_numbering(def_reg):
                    if mnemonic not in ['add','shl','shr','and','or','sub','xor']: # these are both def and a use, just dont renumber for now :((
                        number += 1
                        register_versions[def_reg] += number # this shit better fucking be 64bits register
                        assert mnemonic in ['mov','movabs','add','movzx','shl','shr','and','not','or','sub','xor','lea','cmovne']
                        operands[0] = operands[0].lower().replace(def_reg, f'_{register_versions[def_reg]}_{def_reg}')
                        if def_reg in ('contextmxcsr', 'mxcsr'): # lmaooooo
                            operands[0] += '.32'
    ...
    def reallocate_registers(block_disasm):
        ...
        for addr, field1, field2, mnemonic, op_str in reversed(block_disasm):
            ...
            # there is no memory (*word ptr) on LHS THANK FUCKING GOD! otherwise mov qword ptr [rax], 5 would be a use of rax
        ...
        for addr, field1, field2, mnemonic, op_str, live_in in block_disasm:
            ...
            for def_reg in def_regs:
                if def_reg.startswith('_'):
                    if def_reg in relabel_map: # for add _x_, we shouldnt relabel it again lol
                        assert mnemonic in ['add','shl','shr','and','or','sub','xor']
                        continue
            ...
            if mnemonic == 'mov' and operands[0] == operands[1] and operands[0] in ['rbx','rcx','rdx','rsi','rdi','rbp','r8','r9','r10','r11','r12','r13','r14','r15']: # this is a nop
                print('dropping nop', hex(addr), mnemonic, operands)
                continue
    ...
    def stack_slot_analysis(block_disasm):
        ...
        for addr, field1, field2, mnemonic, op_str in block_disasm:
            ...
            if addr == 0: # synthetic instructions
                if mnemonic == 'mov' and operands[0] == 'rdx' and operands[1] in rsp_refs:
                    rdx_value = rsp_refs[operands[1]]
                    rdx_is_concrete = False
                    emit = False
                    print('rdx is now symbolic')
                elif mnemonic == 'add' and operands[0] == 'rdx':
                    operand = operands[1]
                    if operand.startswith('0x'):
                        offs = int(operand,16)
                    else:
                        offs = int(operand)
                    if not rdx_is_concrete:
                        rdx_value -= offs
                        print(f'rdx -= {offs/8} slots')
                        emit = False
                    else:
                        rdx_value += offs
                        print(f'rdx += {offs} bytes')
                elif mnemonic == 'mov' and operands[1] == 'qword ptr [rdx]':
                    if not rdx_is_concrete:
                        if rdx_value % 8: # ohj my fucking god.
                            slot1 = (rdx_value//8)+1
                            slot2 = slot1 - 1
                            frac = -rdx_value % 8
                            nbits1 = frac*8
                            nbits2 = 64-nbits1
                            print(f'{operands[0]} = (_stackslot{slot1}_ >> {nbits1}) | (_stackslot{slot2}_ << {nbits2});')
                            temp_count += 1
                            # ok, so i think the challenge doesn't actually use the upper bits, so lets just ignore it
                            # otherwise reg alloc fails due to too much long range interferences
                            # result.append((0, 0, 0, 'mov', f'_{temp_count}_temp, _stackslot{slot1}_'))
                            # result.append((0, 0, 0, 'shr', f'_{temp_count}_temp, {nbits1}'))
                            # result.append((0, 0, 0, 'mov', f'{operands[0]}, _stackslot{slot2}_'))
                            # result.append((0, 0, 0, 'shl', f'{operands[0]}, {nbits2}'))
                            # result.append((0, 0, 0, 'or', f'{operands[0]}, _{temp_count}_temp'))
                            result.append((0, 0, 0, 'mov', f'{operands[0]}, _stackslot{slot1}_'))
                            result.append((0, 0, 0, 'shr', f'{operands[0]}, {nbits1}'))
                            if operands[0] == 'rdx':
                                rdx_is_concrete = True
                                print('rdx is now concrete')
                            emit = False
                        else:
                            slot = rdx_value//8
                            print(f'{operands[0]} = _stackslot{slot}_; //')
                            mnemonic = 'mov'
                            operands = [operands[0], f'_stackslot{slot}_']
                            if operands[0] == 'rdx':
                                if operands[1] in rsp_refs:
                                    rdx_value = rsp_refs[operands[1]]
                                else:
                                    rdx_is_concrete = True
                                    print('rdx is now concrete')
                    else:
                        print(f'Concrete dereference on rdx')
                for i in range(len(operands)):
                    if i > 0 and 'rdx' == operands[i]:
                        assert rdx_is_concrete
            else:
                if mnemonic == 'push':
                    rsp_refs['rsp'] += 8
                    assert rsp_refs['rsp'] % 8 == 0
                    slot = rsp_refs['rsp'] // 8
                    print('push', f'_stackslot{slot}_ = {operands[0]}')
                    mnemonic = 'mov'
                    operands = [f'_stackslot{slot}_', operands[0]]
                
                for i in range(len(operands)):
                    if 'qword ptr [rsp + 0x' in operands[i]:
                        offs = int(operands[i].split('qword ptr [rsp + 0x')[1].split(']')[0],16)
                        assert offs % 8 == 0
                        assert offs <= rsp_refs['rsp']
                        slot = (rsp_refs['rsp']-offs)//8
                        operands[i] = f'_stackslot{slot}_'
                        print('reference', f'_stackslot{slot}_')
                    elif 'qword ptr [rsp]' == operands[i]:
                        slot = rsp_refs['rsp'] // 8
                        operands[i] = f'_stackslot{slot}_'
                        print('reference', f'_stackslot{slot}_')

            if mnemonic == 'mov' and operands[1] in rsp_refs:
                rsp_refs[operands[0]] = rsp_refs[operands[1]]
                print(f'stackref: {operands[0]} = slot {rsp_refs[operands[1]]/8}')
            elif mnemonic in ('mov', 'movabs') and operands[0] in rsp_refs:
                del rsp_refs[operands[0]]
                print(f'{operands[0]} is no longer a stack ref')
                if operands[0] == 'rdx':
                    rdx_is_concrete = True
                    print('rdx is now concrete')
            ...
            if emit:
                ...
                if 'rsp' in op_str:
                    continue
                result.append((addr, field1, field2, mnemonic, op_str))
        return result
    ...
    if is_new_code:
        ...
        emulate(pBigly, pFlagBuf)
        open('KMS_TRACE.json','w').write(json.dumps(block_disasm))
        block_disasm = json.loads(open('KMS_TRACE.json','r').read())

    print(len(block_disasm))

    block_disasm = clean_disasm(block_disasm)

    open('KMS_POST_CLEAN.json','w').write(json.dumps(block_disasm))
    block_disasm = json.loads(open('KMS_POST_CLEAN.json','r').read())
    print('code is now clean',len(block_disasm))

    block_disasm = [(0,0,0,'mov', 'mxcsr, 0')] + block_disasm

    block_disasm = global_value_numbering(block_disasm)

    open('KMS_POST_GVN.json','w').write(json.dumps(block_disasm))
    block_disasm = json.loads(open('KMS_POST_GVN.json', 'r').read())
    print('code is now GVN', len(block_disasm))

    block_disasm = reallocate_registers(block_disasm)

    open('KMS_POST_REGALLOC.json','w').write(json.dumps(block_disasm))
    block_disasm = json.loads(open('KMS_POST_REGALLOC.json', 'r').read())
    print('code is now REGALLOC', len(block_disasm))

    block_disasm = stack_slot_analysis(block_disasm)

    open('KMS_POST_STACKSLOT.json','w').write(json.dumps(block_disasm))
    block_disasm = json.loads(open('KMS_POST_STACKSLOT.json', 'r').read())
    print('code is now STACKSLOT', len(block_disasm))

    block_disasm = reallocate_registers(block_disasm)
    open('KMS_POST_FINAl.json','w').write(json.dumps(block_disasm))
    block_disasm = json.loads(open('KMS_POST_FINAl.json', 'r').read())
    print('code is now FINAl', len(block_disasm))
    ...
    open('new_code.asm','w').write(new_asm)
